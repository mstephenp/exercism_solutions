defmodule ListOpsTest do
  ...
  test "append of huge lists" do
    assert L.append(Enum.to_list(1..1000), Enum.to_list(1001..2000)) ==
             Enum.to_list(1..2000)
  end
  ...
  test "concat of huge list of small lists" do
    assert L.concat(Enum.map(1..100, &[&1])) == Enum.to_list(1..100)
  end

  # @tag :pending
  test "concat of small list of huge lists" do
    assert L.concat(Enum.map(0..9, &Enum.to_list((&1 * 10 + 1)..((&1 + 1) * 10)))) ==
             Enum.to_list(1..100)
  end
end
